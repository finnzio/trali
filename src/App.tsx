...
const EDGE_DOCK_KEY = "translator.edgeDockEnabled";
  ...
  return (
    <Select value={value} onValueChange={(next) => onValueChange(String(next))}>
      <SelectTrigger
        className={
          triggerClassName ??
          "min-w-36 border-0 bg-transparent shadow-none"
        }
      >
        <SelectValue>
          {value === "auto"
            ? (autoLabel ?? "Detect language")
            : languageName(value)}
        </SelectValue>
      </SelectTrigger>
      <SelectContent align="start">
        {includeAuto && (
          <SelectItem value="auto">{autoLabel ?? "Detect language"}</SelectItem>
        )}
...
function App() {
  ...
  const [edgeDockSupported, setEdgeDockSupported] = useState(
    () => !IS_LINUX,
  );
  const [edgeDockEnabled, setEdgeDockEnabled] = useState(
    () =>
      !IS_LINUX &&
      window.localStorage.getItem(EDGE_DOCK_KEY) !== "false",
  );
  ...
  useEffect(() => {
    if (!isTauri()) return;
    void invoke("set_edge_dock_enabled", {
      enabled: edgeDockSupported && edgeDockEnabled,
    }).catch(() => {});
  }, [edgeDockEnabled, edgeDockSupported]);

  useEffect(() => {
    void invoke<boolean>("edge_dock_supported")
      .then((supported) => {
        setEdgeDockSupported(supported);
        setEdgeDockEnabled((current) => {
          const enabled = supported && current;
          void invoke("set_edge_dock_enabled", { enabled });
          return enabled;
        });
      })
      .catch(() => {
        // Browser preview uses the platform-derived initial state.
      });
  }, []);
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (activeGenerationIdRef.current) {
        void cancelGeneration(activeGenerationIdRef.current).catch(() => {});
    ...
    const timeout = window.setTimeout(() => {
      ...
      const onEvent = createGenerationChannel((event: GenerationEvent) => {
        if (event.requestId !== activeGenerationIdRef.current) return;
      ...
      void generate(
        {
          ...
          variants: selectedStyleIds.map((id) => ({
            id,
  ...
  function applyBackendSettings(settings: AppSettings) {
    ...
    setEdgeDockEnabled(Boolean(settings.edgeDockEnabled));
        ...
        <div className="mx-auto w-full max-w-2xl px-3 py-2 sm:px-4">
          <header className="flex items-center gap-3">
            <Button
              ...
              className="h-auto gap-1 px-1 py-1 font-semibold"
            >
          ...
          <div
            className="mt-2 grid w-fit grid-cols-4 rounded-md bg-muted p-0.5"
            role="tablist"
            ...
            {settingsTab === "preferences" && (
              <div className="grid max-w-xl gap-3">
                ...
                <div className="order-7 grid grid-cols-[7rem_1fr] items-center gap-3">
                  <Label>{t("edgeDock")}</Label>
                  <div className="flex min-h-9 items-center gap-2">
                    <Switch
                      checked={edgeDockEnabled}
                      disabled={!edgeDockSupported}
                      onCheckedChange={(checked) => {
                        setEdgeDockEnabled(checked);
                        void invoke("set_edge_dock_enabled", {
                          enabled: checked,
                        }).catch(() => {});
                      }}
                      aria-label={t("edgeDock")}
                    />
                    {!edgeDockSupported && (
  ...
  return (
    <main className="h-svh overflow-hidden bg-card">
      <div className="h-full w-full">
        <section className="grid h-full grid-rows-[minmax(0,1fr)_auto_minmax(0,1fr)] overflow-hidden bg-card">
          <div className="grid min-h-0 grid-rows-[auto_1fr]">
                ...
                <div className="flex rounded-lg bg-muted p-0.5">
                  {(["translate", "proofread"] as WorkMode[]).map((mode) => (
                    <Button
                      ...
                      className={
                        workMode === mode
                          ? "h-7 bg-background px-2.5 shadow-sm"
                          : "h-7 px-2.5 text-muted-foreground"
                      }
                      onClick={() => {
                        setWorkMode(mode);
                      }}
            ...
            <Textarea
              ...
              className="h-full min-h-0 resize-none rounded-none border-0 px-5 py-4 text-base shadow-none focus-visible:ring-0"
              autoFocus
            />
          </div>
          ...
          <div className="relative">
            <Separator />
            <div className="absolute top-1/2 left-1/2 flex size-8 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border bg-card text-muted-foreground shadow-xs">
            ...
            <div className="flex items-center justify-between px-4 py-2.5">
              {workMode === "translate" ? (
                ...
                <span className="text-sm font-medium">
                  {t("writingSuggestions")}
                </span>
              )}
              <div
                className="flex min-w-0 items-center justify-end gap-1 overflow-x-auto"
                  ...
                  <Button
                    ...
                    className={
                      selectedStyleIds.includes("default")
                        ...
                        : "h-7 shrink-0 rounded-full px-2.5 text-muted-foreground"
                    }
                    onClick={() => {
                      setSelectedStyleIds((current) => {
              ...
              >
                {translationVersions.map((version) => (
                  ...
                  >
                    ...
                    <div className="relative">
                      <div className="whitespace-pre-wrap px-5 py-4 pr-24 text-base">
                        {version.error ? (
                      ...
                      <div className="absolute top-3 right-4 flex items-center gap-1">
                        ...
                        <Button
                          ...
                          size="icon-sm"
                          onClick={() =>
                            void copyTranslation(version.id, version.text)
                          }
